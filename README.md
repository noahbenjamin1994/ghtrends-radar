...
Deep research covers four search jobs: official products/pricing, user problems, independent community discussion and open-source implementations. It combines the existing residential search route with public sources, reading up to eight independent original-page domains with four concurrent workers and a 40-second overall reading budget. Light research retains four domains and 20 seconds. Community queries follow the selected direction. The report shows actual original reads, discussion records, source websites and unavailable reads; these counts do not represent distinct buyers or willingness to pay.
...
Standard research uses a compact direct-generation blueprint and overlaps independent source collection and report sections. A source-bound capability audit precedes up to five parallel direction writers, market coverage and request interpretation. The selected pilot and summary are independent. There is no mandatory full-report second review: exact citations, evidence scope, bilingual structure and shared experiment criteria remain delivery gates, with targeted semantic review when a concrete check fails. Style checks preserve technical terms such as untrusted inputs and immutable logs. `GHTRENDS_RESEARCH_THINKING=low|off` and `GHTRENDS_RESEARCH_REVIEW_THINKING=low|off` independently configure blueprint and on-demand review reasoning; both default to `off`. Both report types stream live activity with polling recovery. Prompts, draft JSON and reasoning text stay on the server. Provider timings and specific validation failures support operator diagnosis; upstream rate limits and format recovery can still extend latency.
