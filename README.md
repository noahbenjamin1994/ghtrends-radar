...
A scan runs up to three first-page queries: commercial alternatives, user problems and open-source assets. Results are cached for six hours; one paced queue and a cooldown after Google challenges (one minute for rotating gateways, 15 minutes for fixed routes) bound recovery attempts. Query, region, collection date, links, snippets and organic/ad labels are retained. Worldwide reports use a clearly labeled US search sample. Only bounded excerpts enter AI research, while credentials remain on the server. Admin records search calls, cache reuse, latency and failures separately; measured direct-request HTTP bytes join residential usage. Mobile results expose the placements actually present on that page; ad visibility is a sample, and an empty ad list leaves advertising activity open.

Reports separate open-source projects, commercial competitors and observed ads. Projects show activity, age and community data. Competitors show who they serve, source-backed pricing or buyback quotes, existing advantages and possible openings. Audience and pricing facts carry individual quotes; trade-in payouts keep their eligibility and device scope. Ads show the destination website, copy, landing page, query and region. Advertiser identity, spend, clicks and conversions require additional evidence. Model inputs prioritize independent websites, then fill remaining slots with same-site pages, with up to four organic and two ad sources per query.
