...
All requests hard-disable thinking, regardless of legacy flags. Deep research combines factual, decision and correction reviews into one call. Invalid review output stops without an extra review; original quote checks remain local. Writing uses at most 12 sources, with model excerpts capped at 2,000 characters (licenses: 6,000), labeled when truncated. Original evidence and interrupted drafts are retained. No automatic stronger-model fallback runs.

**Free light briefs and deep research**

New light briefs deliver exactly three directions, with a concise judgment, comparison and next step that follows the selected direction. Details and evidence open on demand; historical reports retain their original direction count. Standard research keeps its free daily allowance.
...
With `GHTRENDS_PUBLIC_SOURCES=1`, the three-query budget is retained: community discovery targets public Reddit, Hacker News and Trustpilot indexes; physical-goods topics replace the open-source query with Amazon, eBay and Etsy discovery. Software topics also run one anonymous HN / Algolia search in parallel (four hits, six-second timeout, six-hour success cache). Deduplicated candidates are capped at 30, reuse the existing relevance review, and contribute at most 16 search sources to writing; original-page reads remain capped at four. No platform account, session cookies or additional paid API is required; existing search proxy usage still applies. Indexed snippets are not complete discussions, reviews or live prices. Listings do not establish sales, authorization or unmet demand. Access challenges remain explicit collection failures; login and CAPTCHA are not bypassed.
...
Standard research uses a compact direct-generation blueprint and overlaps independent source collection and report sections. A source-bound capability audit precedes up to five parallel direction writers, market coverage and request interpretation. The selected pilot and summary are independent. There is no mandatory full-report second review: exact citations, evidence scope, bilingual structure and shared experiment criteria remain delivery gates, with targeted semantic review when a concrete check fails. Style checks preserve technical terms such as untrusted inputs and immutable logs. Thinking is hard-disabled regardless of legacy flags. Both report types stream live activity with polling recovery. Prompts, draft JSON and reasoning text stay on the server. Provider timings and specific validation failures support operator diagnosis; upstream rate limits and format recovery can still extend latency.
...
Successful `sources[].links` includes up to 12 same-site pricing/documentation/features/license navigation hints, not verified evidence or automatic crawl targets. Search always returns a boolean `cached`; cached pages keep their original observation dates. Batch workers schedule origin groups so one site's URLs cannot occupy all slots.

Two pre-release residential-proxy samples on 2026-09-24: cold search 3.7–5.1 seconds, two-page cold batches 9.1–14.6 seconds; warm search 15–16 ms and batches 8 ms measured over server loopback (excluding public client round trips). One robots timeout recovered. These samples verify connectivity, not a long-term success rate or P95 guarantee. No LLM calls were made.
...
Output ceilings (tokens): planning 900; relevance/copy 2,200; evidence/issues 3,500; default 4,000; deep write/repair/review 4,500/3,000/1,800; legacy full bilingual recovery 9,000. Copy batches have at most six fields. Context-tracked runs stop at 24 calls (deep: 12) or 60,000 reserved output tokens. Unscoped CLI calls retain per-request caps. These are ceilings, not targets or currency guarantees. Cost-policy tests use mocks, not paid generations.
