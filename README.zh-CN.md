...
交付前另做一次聚焦决策的独立审查：核对竞品是否服务同一买家与任务、所谓差异是否已经存在、实验是否真能支持结论。该检查使用有界深度思考，失败不会绕过；修复仍受同一轮数上限约束。缺乏买家依据时可以明确建议暂缓，并给出具体的工作流程核验；访谈、一次付款与持续订阅需求不能混为一谈。
...
深度初稿、实质返修与语义复核默认使用有界轻量思考；查询整理和短文压缩保持直接生成。`GHTRENDS_DEEP_WRITE_THINKING=off`、`GHTRENDS_DEEP_REVIEW_THINKING=off` 可用于明确的成本/质量对照。`GHTRENDS_DEEP_MODEL` 可单独选择更强的深度模型，免费轻量研究仍使用 `DEEPSEEK_MODEL`。来源事实与“对你的意义”分别展示，投入估算使用统一的总人时区间。每份专项说明围绕所选问题整理 2–4 条关键发现。 新专项说明与普通报告共用实验数字规则，由参与人数和任务数生成两种语言的行动门槛。引用项目原文中的使用说明同时显示在资源区和 Markdown 导出中。普通报告、个人筛选与专项说明的措辞修复同时参考中英原句，保留数字门槛、角色与来源归属；引用精简继续保留原文措辞。程序只接收指定字段的返修；修订复核同时带入原问题及改动前后内容，原话另做逐字校验。思考复核遇到输出预算或格式问题时，最多补一次关闭思考的短复核，沿用相同的来源与交付检查；供应商限流按恢复状态处理。涉及事实的修改意见先对照原文核验，再执行定点返修；写作与复核使用同一组有界证据。有效草稿随任务保存，复核中断后按原来源和用户条件继续，草稿与内部意见保留在服务端。项目许可原文最多保留 20,000 字符，超出时明确标记摘录；源码许可与数据复用条件分别核对。关键原文需要补充时，保留已有证据并返还首次体验次数。公开摘录和选填背景会发送给配置的研究模型。
...
普通研究默认直接生成紧凑提纲，原始资料采集与独立章节并行处理。先核对产品能力，再同时整理最多五个方向、市场概览与社区请求；优先方向的实验与摘要独立生成。取消固定的整篇二次复核，交付继续检查原文引用、证据范围、中英文结构及共享实验条件，具体语义问题才触发定点复核。风格检查保留“不可信”“未经授权”等有明确含义的安全术语。`GHTRENDS_RESEARCH_THINKING=low|off` 和 `GHTRENDS_RESEARCH_REVIEW_THINKING=low|off` 分别控制提纲及按需复核的轻量思考，默认均为 `off`。普通报告和专项研究实时展示请求、生成与完成状态；连接恢复时自动查询进度。提示词、草稿 JSON 与推理原文保留在服务端。阶段调用和具体校验失败原因供管理员排查；外部服务限流或格式修复仍可能增加耗时。
