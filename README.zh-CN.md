...
专项研究在现有摘录预算内，优先读取所选项目 README 与发布说明中相关的正文段落。网页原文沿用四个域名的预算，与所选方向匹配的文档链接优先于一般比较文章，公开讨论的优先级与来源身份继续保留。网址与词语匹配用于安排读取顺序，来源权威性与事实支持仍需核对。每页下载上限为 2 MB，正文摘录上限为 6,000 字符，超出时标记截取状态。简介与选定原文保留来源链接、采集日期，省略标记和截取状态说明覆盖范围。清理展示标签时保留 Markdown 代码块、行内代码和自动链接，包括 Shell 重定向语法。
