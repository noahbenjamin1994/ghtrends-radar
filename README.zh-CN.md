...
深度推理先形成精炼蓝图，再分别整理双语细分方向与整体判断；细分方向最多同时整理两项，每项校验并缓存。整体生成后再复核证据范围、实际功能与建议服务的区别，以及双语含义。超长文字单独精简，社区请求通过独立步骤读取。管理员可分别查看各环节的调用与消耗。
...
3. 首轮高强度思考先产出精简研究提纲。每个方向分别进行 1 次 GitHub 项目检索、最多 1 份 README 阅读、1 次 Issue 检索（最多 3 条摘录），由两个采集任务并行处理。证据编辑结合提纲与新来源生成完整双语报告，再校验引用与表达。这让深入判断和完整成稿各有独立的输出空间。
