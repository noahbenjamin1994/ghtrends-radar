...
查询整理、写作与措辞修复关闭 thinking；专项研究的事实复核默认使用轻量思考，`GHTRENDS_DEEP_REVIEW_THINKING=off` 可用于明确的成本/质量对照。来源事实与“对你的意义”分别展示，投入估算使用统一的总人时区间。措辞修复仅调整对应字段，随后核对原话和实质性来源支持。关键原文需要补充时，保留已有证据并返还首次体验次数。公开摘录和选填背景会发送给配置的研究模型。
