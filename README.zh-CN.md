...
报告的“核对原文与许可”保留采集日期、原文摘录与读取状态，Markdown、JSON 和静态 HTML 同步包含。官网数据代表发布方声明；用户讨论代表个人经历。网页读取遵守 robots.txt，限制跳转、耗时与体积，连接固定到已核验的公网地址。页面支持 gzip、Brotli 与 deflate 压缩传输，传输与解压体积各限 2 MB，保存摘录保持 6,000 字符上限。临时连接故障在共享 20 秒读取期限内补试一次，并遵守站点抓取间隔。原文缓存一小时；访问受限时保留搜索摘要与状态。来源请求走独立的公开读取通道，后端身份凭据仅用于固定的 GitHub API。设置 `GHTRENDS_SOURCE_DOCUMENTS=0` 可关闭这一层采集。Reddit、X 的正文接入继续按各平台授权条件推进。
