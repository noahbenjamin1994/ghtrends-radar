...
查询整理、写作与措辞修复关闭 thinking；专项研究的事实复核默认使用轻量思考，`GHTRENDS_DEEP_REVIEW_THINKING=off` 可用于明确的成本/质量对照。来源事实与“对你的意义”分别展示，投入估算使用统一的总人时区间。每份专项说明围绕所选问题整理 2–4 条关键发现。程序只接收指定字段的返修；修订复核同时带入原问题及改动前后内容，原话另做逐字校验。思考复核遇到输出预算或格式问题时，最多补一次关闭思考的短复核，沿用相同的来源与交付检查；供应商限流按恢复状态处理。涉及事实的修改意见先对照原文核验，再执行定点返修；写作与复核使用同一组有界证据。有效草稿随任务保存，复核中断后按原来源和用户条件继续，草稿与内部意见保留在服务端。项目许可原文最多保留 20,000 字符，超出时明确标记摘录；源码许可与数据复用条件分别核对。关键原文需要补充时，保留已有证据并返还首次体验次数。公开摘录和选填背景会发送给配置的研究模型。
