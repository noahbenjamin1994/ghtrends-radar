...
社区请求解读、资料提取与正文整理直接生成；方向构思与最终证据复核保留轻量思考，使用 `GHTRENDS_RESEARCH_REVIEW_THINKING=low|off` 单独配置。`GHTRENDS_RESEARCH_THINKING=low` 是方向提纲的默认设置；设为 `off` 且省略独立复核设置时，复核也直接生成。独立章节以两路并行整理。普通报告和专项研究实时展示请求、生成与完成状态；连接恢复时自动查询进度。提示词、草稿 JSON 与推理原文保留在服务端。交付继续核对原话、事实范围和中英文含义。管理员可查看最终复核模式，以及输出 Token 中的思考用量。
