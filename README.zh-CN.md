...
**账户与次数：** `/account` 分别展示每日研究、首次专项体验和已购研究包。托管部署通过服务端的 `GHTRENDS_NEXUS_URL`、`GHTRENDS_NEXUS_PROJECT_ID`、`GHTRENDS_NEXUS_PROJECT_KEY` 连接 Nexus，登录后查看购买确认、每笔到期日和次数流水。记录区分预留、完成、释放、到期及购买调整；连接恢复期间显示同步提示和历史记录时间。此连接用于账户展示，正式任务扣次及购买入口按支付联调里程碑开启。
