...
任务先保存，实际执行时预留次数；等待中的任务可取消。完整结果与结算意图在 SQLite 同时保存，运行中断时释放原购买预留并沿用原到期日。回复丢失或服务重启后，系统通过同一 Nexus 凭据继续核对；删除研究正文仍保留待处理账务。管理员面板展示处理中结算与需要人工核对的记录。执行检查点限制为 15 分钟，预留时限为一小时。此实现用于单个持久应用实例；实际结账与生产启用继续按支付和研究质量验收推进。
