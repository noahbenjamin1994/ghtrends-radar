...
托管账户首次获赠 1 次专项体验，按账户终身记录，与普通研究的每日额度分别计算。开始时预留，完整交付后使用；阶段性交付或服务重启时返还次数。自部署使用运营者自己的服务密钥。相同请求自动去重；任务可从私有历史恢复，同一任务最多尝试 3 次，每个账户每天最多尝试 3 次。`GHTRENDS_DEEP_DAILY_REQUESTS` 配置每日新任务容量，默认 20。阅读、Markdown/JSON 导出消耗 0 次；用户可删除研究内容，最小使用记录继续保留尝试数与已使用次数。预览功能可单独配置，研究质量和成本继续验收，研究包购买按后续里程碑推进。

写作与措辞修复关闭 thinking；措辞修复仅调整对应字段，随后核对原话和实质性来源支持。关键原文需要补充时，保留已有证据并返还首次体验次数。公开摘录和选填背景会发送给配置的研究模型。
