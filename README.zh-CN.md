...
`GHTRENDS_RESEARCH_THINKING=low` 为默认模式：研究提纲与独立证据复核采用轻量思考；正文整理与社区请求解读直接生成。措辞校订优先直接生成，反复校验失败的少数字段再进行有界的轻量思考修订。设置 `off` 后全流程直接生成，可用自有主题对比质量与消耗。模型上下文省去重复的引用元信息与译文，存储的来源证据完整保留。管理员可查看当前模式，以及输出 Token 中的思考用量；历史缺省明细标为待补充。
