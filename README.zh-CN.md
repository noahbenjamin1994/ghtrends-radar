...
深度研究围绕一个方向和一个问题，规划四组查询（官方产品/价格、用户问题、独立社区讨论、开源实现），结合现有住宅代理搜索和公开来源。网页原文最多读取八个独立域名，四路并发、40 秒总读取预算；轻量层仍为四个域名、20 秒。社区查询使用所选方向。页面展示实际原文、讨论记录、来源网站及受限读取状态；这些计数不表示独立买家或付费意愿。
...
查询整理、写作与措辞修复关闭 thinking；专项研究的事实复核默认使用轻量思考，`GHTRENDS_DEEP_REVIEW_THINKING=off` 可用于明确的成本/质量对照。来源事实与“对你的意义”分别展示，投入估算使用统一的总人时区间。每份专项说明围绕所选问题整理 2–4 条关键发现。 新专项说明与普通报告共用实验数字规则，由参与人数和任务数生成两种语言的行动门槛。引用项目原文中的使用说明同时显示在资源区和 Markdown 导出中。普通报告、个人筛选与专项说明的措辞修复同时参考中英原句，保留数字门槛、角色与来源归属；引用精简继续保留原文措辞。程序只接收指定字段的返修；修订复核同时带入原问题及改动前后内容，原话另做逐字校验。思考复核遇到输出预算或格式问题时，最多补一次关闭思考的短复核，沿用相同的来源与交付检查；供应商限流按恢复状态处理。涉及事实的修改意见先对照原文核验，再执行定点返修；写作与复核使用同一组有界证据。有效草稿随任务保存，复核中断后按原来源和用户条件继续，草稿与内部意见保留在服务端。项目许可原文最多保留 20,000 字符，超出时明确标记摘录；源码许可与数据复用条件分别核对。关键原文需要补充时，保留已有证据并返还首次体验次数。公开摘录和选填背景会发送给配置的研究模型。
