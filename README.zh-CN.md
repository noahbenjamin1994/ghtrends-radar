...
| 活跃开源供给 | 持续搜索增速 | 赛道判断 |
|---|---|---|
| 少 | 快 | **早期蓝海**：验证尚未被充分服务的场景 |
| 多 | 快 | **增长型红海**：寻找具体受众或竞争优势 |
| 多 | 未达到快速增长标准 | **存量红海**：找到用户愿意迁移的理由 |
| 少 | 未达到快速增长标准 | **静水区**：验证是早期、小众还是缺乏活力 |
| 证据缺失或过弱 | 未确认 | **初步建议**：明确已知事实、建议行动和验证步骤，保留象限未定状态 |
...
扫描会显示来源进度，在项目详情补充完毕前先展示初步结果。用户扫描优先于定时刷新，仓库详情最多使用三个并发请求；数据源限流仍可能增加等待时间。当前无需 LLM 或付费搜索服务。

## 60 秒开始使用

**直接打开网站：** [radar.ghtrends.dev](https://radar.ghtrends.dev/?lang=zh)，无需注册。
...
- **网页**：机会地图、赛道证据、仓库历史、竞品对比和浏览器关注清单。可导出 PNG、Markdown、JSON，也可将固定报告卡片嵌入 README。
...
**供给：** 查询选定 GitHub 主题，已识别领域可以组合多个明确列出的主题检索。仓库须未归档、不是 Fork、至少 5 Star，且 180 天内有提交。目前以 50 个符合条件的项目作为供给密集门槛。单主题采用搜索返回的数量，展示最多 100 个主要项目；多主题对返回项目去重，未完整枚举时给出下限。它不是全市场竞品普查，不包含所有无标签或闭源产品。

**需求：** 同一请求中获取两年 Google Trends 数据与共同参考词 `github trending`。对比最近 8 个完整周与此前 8 周的中位数。快速增长必须同时满足：

1. 至少 26 个连续完整周，并有足够的非零观测。
2. 前期中位数至少为 3，避免接近零的分母。
3. 中位数增长至少 25%。
4. 确定性的两周分块重采样区间下界为正。
5. 最近 8 周至少 6 周超过原中位数的 112.5%。
6. 未被识别为重复去年的季节性反弹。

证据缺失、过旧、不连续、接近零或采集失败时，机器可读分类保留 `uncertain`，展示层给出明确的初步建议。即使来源未标记未完成周，也只使用采集与分析时均已结束的完整周。无效新行不能让旧数据变新；同周冲突值会阻止分类。重采样只检查稳定性，不代表成功概率。50 项目的门槛和机会分数权重仍需跨赛道实证校准。
...
前端采用 React + TypeScript + Vite；核心为 TypeScript、Express、SQLite、Commander 与官方 MCP SDK。单包结构，无需外部数据库或 LLM Key。
...
公共部署设置 `GHTRENDS_HOSTED=1`，首页仅展示精选赛道，自定义扫描仍保留固定报告链接。关键词变体分开存储，不会覆盖标准赛道。配置 `PUBLIC_URL`；按需启用 `GHTRENDS_AUTO_COLLECT=1`，根据来源采集时间每日刷新精选赛道。`TRUST_PROXY` 仅填写实际受信任的反向代理网络。公共扫描限频，用户任务优先于定时采集；重新解释报告不会推迟数据刷新。
