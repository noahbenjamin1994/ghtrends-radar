...
import { validQuote, sourceQuoteSchema } from "./landscape.js";
...
facts: copy up to four exact short original-document substrings that matter to THIS proposed job (prefer 8–300 characters; complete Chinese statements may have 4–7 characters, hard maximum 600 when keeping a complete statement or its qualifiers). Each fact keeps its supplied source ID. Select relevant implemented features and explicit requirements. FIRST look for contradictions and hard prerequisites: discontinued or winding-down services, restricted new-user access, version-specific capability limits, unavailable endpoints, and authorization constraints. Quote any relevant restriction before favorable feature or pricing statements. General model pricing or release announcements do not establish training, self-hosting or resale capability; an unrelated model/version cannot establish it either. If a required capability is unsupported or unverified, proposedWork must begin with a feasibility check or a documented alternative, and nextCheck must resolve that prerequisite before implementation. Empty facts with overlap to-check are valid when no original document supports this job. Never cite an unrelated project merely because its name or keyword matches; robotics code cannot establish entertainment-IP licensing rights. Keep those missing facts as nextCheck. A README copyright reservation is a publisher statement requiring permission before reuse; a README license label calls for checking the actual license file. A supplied license file establishes conditions for its own asset. Keep each project's identity, versions, dates and testing-only restrictions. directionId describes why a document was collected; a relevant original document can inform several directions. A README describes a maintainer claim; third-party review pages describe the reviewer's claims and call for publisher verification. Collected metadata and search snippets are discovery leads. Individual requests establish requests, while product documents establish implemented features. Preserve literal source text including Markdown punctuation. Quotes carry the observation directly; keep paraphrased feature claims out of this record.
