...
export function renderDocument(
  ...
  if (m && assessment) {
    ...
    content = `<p class="eyebrow">${e("CATEGORY INTELLIGENCE /")} ${e(m.geo || "WORLDWIDE")}</p><h1>${e(m.topic.name)}</h1><p>${e(m.topic.description)}</p>
      ...
      <p>${e("Measured search term")}: ${escapeHtml(m.demand.keyword)} · ${escapeHtml(assessment.demandNote)}</p>${list(assessment.facts)}${m.kind === "uncertain" ? `<p>${e("Quadrant not yet established")}</p><h3>${e("What to do next")}</h3>${list(assessment.nextSteps)}` : ""}
