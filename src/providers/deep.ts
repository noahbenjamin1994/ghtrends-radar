...
const reviewSchema = z
  .object({
    ready: z.boolean(),
    corrections: z.array(z.string().max(1200)).max(8),
  })
  .strict();
...
export async function runDeepResearch(
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    ...
    for (let copyPass = 0; copyPass < 2; copyPass++) {
      const fields = deepCopyRepairs(candidate);
...
All plan fields are displayed as research proposals. New artifact designs, participant counts, hours and decision thresholds are proposed values, not historical facts. Check their fit to the user context and label genuine resource dependencies. Existing conditional or inferred wording already serves that purpose. Vendor audiences remain vendor claims; search snippets remain leads; open requests remain proposals. The answer should address the selected question with a specific next action, supported factual premises and explicit checks.
